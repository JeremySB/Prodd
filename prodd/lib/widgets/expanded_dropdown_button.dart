...
// Copied from Flutter's dropdown.dart file, with small modifications to make the DropdownButton fill width
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  final List<ExpandedDropdownMenuItem<T>> items;
...
/// An item in a menu created by a [ExpandedDropdownButton].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class ExpandedDropdownMenuItem<T> extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const ExpandedDropdownMenuItem({
    Key key,
    this.value,
    @required this.child,
  }) : assert(child != null),
  ...
  ///
  /// Eventually returned in a call to [ExpandedDropdownButton.onChanged].
  final T value;
...
/// Typically, an enum is used. Each [ExpandedDropdownMenuItem] in [items] must be
...
/// See also:
///
///  * [ExpandedDropdownMenuItem], the class used to represent the [items].
...
class ExpandedDropdownButton<T> extends StatefulWidget {
  ...
  ExpandedDropdownButton({
    ...
    this.isDense = false,
       ...
       assert(value == null || items.where((ExpandedDropdownMenuItem<T> item) => item.value == value).length == 1),
      super(key: key);
...
class _DropdownButtonState<T> extends State<ExpandedDropdownButton<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(ExpandedDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    assert(widget.value == null ||
      widget.items.where((ExpandedDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
